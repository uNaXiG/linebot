...
TownNameFile = os.path.join(path, 'location_report_data.csv')
WeatherFile = os.path.join(path, 'Weather', f"{datetime.datetime.now().strftime('%Y_%m_%d')}_data", f"{datetime.datetime.now().strftime('weather_report')}_data.csv")
...
MovieIdFile = os.path.join(os.getcwd(), 'Movie', 'movie_id.csv')
...
uidFile = os.path.join(os.getcwd(), 'UID_Data', 'user_id.txt')
uid_arr = []
...
def SendMsg(loc_town, tempMsg):
    with open(WeatherFile, newline='', encoding='utf-8') as csvfile:
